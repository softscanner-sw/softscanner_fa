...
export function generateReport(
  ...
  lines.push('');
  lines.push(`| # | Test File | Outcome | Attempts | Duration | Error |`);
  lines.push(`|---|---|---|---|---|---|`);
  for (let i = 0; i < b3.results.length; i++) {
    const r = b3.results[i]!;
    const shortFile = r.testFile.replace(/\.test\.ts$/, '');
    const errorSnippet = r.error ? r.error.slice(0, 80).replace(/\|/g, '\\|').replace(/\n/g, ' ') : '';
    lines.push(`| ${i + 1} | ${shortFile} | ${r.outcome} | ${r.attempts} | ${r.durationMs}ms | ${errorSnippet} |`);
  }
  lines.push('');
  ...
  if (failures.length > 0) {
    ...
    // B5.0 failureKind breakdown (from per-test structured logs if available)
    ...
    if (fs.existsSync(logsDir)) {
      ...
      if (logFiles.length > 0) {
        ...
        for (const lf of logFiles) {
          try {
            const log = JSON.parse(fs.readFileSync(path.join(logsDir, lf), 'utf-8'));
            if (log.outcome === 'FAIL' && log.failureKind) {
              byKind.set(log.failureKind, (byKind.get(log.failureKind) ?? 0) + 1);
            }
